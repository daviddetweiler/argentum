...
namespace argentum::sha256 {
	namespace {
		...
		void hash_block(hash_state state, gsl::span<const std::uint8_t, block_bytes> block) noexcept
		{
			std::array<std::uint32_t, 64> w {};
			std::memcpy(w.data(), block.data(), block.size());

			for (auto t = 0ull; t < w.size(); ++t) {
				auto& ws = gsl::at(w, t);
				if (t < 16)
					ws = be(ws);
				else
					ws = sigma1(gsl::at(w, t - 2ul)) + gsl::at(w, t - 7ull) + sigma0(gsl::at(w, t - 15ull))
						+ gsl::at(w, t - 16ull);
			}
	...
	void stomach::init() noexcept
	{
		Expects(position == sentinel);
		this->hash = first_hash;
		position = 0;
	}

	void stomach::complete(digest d) noexcept
	{
		{
			Expects(position != sentinel);

			const auto bits = be(position * 8);
			const auto after_marker = position + 1;
			const auto index = (after_marker - 56) & block_mask;
			const auto padding_needed = (block_len - index) & block_mask;
			append(gsl::span {padding}.subspan(0, padding_needed + 1));
			append(gsl::span {to_uint8_tp(&bits), sizeof(std::uint64_t)});
			for (auto& s : this->hash)
				s = be(s);

			std::memcpy(d.data(), this->hash.data(), d.size());
			position = sentinel;
		}
	}

	void stomach::append(gsl::span<const std::uint8_t> data) noexcept
	{
		Expects(position != sentinel);
		while (data.size()) {
			const auto index = position & block_mask;
			const auto remaining = block_len - index;
			if (remaining == block_len && data.size() > block_len) {
				hash_block(hash, data.subspan<0, block_len>());
				data = data.subspan<block_len>();
				position += block_len;
			}
			else {
				const auto tail_len = std::min(remaining, data.size());
				std::memcpy(std::next(pending_block.data(), index), data.data(), tail_len);
				if (tail_len == remaining)
					hash_block(hash, pending_block);

				data = data.subspan(tail_len);
				position += tail_len;
			}
		}
	}
